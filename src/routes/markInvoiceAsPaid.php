...
$app->post('/api/PayPal/markInvoiceAsPaid', function ($request, $response, $args) {
    ...
    if(isset($post_data['args']['sandbox']) && $post_data['args']['sandbox'] == 1) {
        $query_str = 'https://api.sandbox.paypal.com/v1/invoicing/invoices/'.$post_data['args']['invoiceId'].'/record-payment';
    } else {
        $query_str = 'https://api.paypal.com/v1/invoicing/invoices/'.$post_data['args']['invoiceId'].'/record-payment';
    }
    ...
    try {
        ...
        if(in_array($code, ['200','201','202','203','204'])) { 
            ...
            $result['contextWrites']['to'] = "marked";
        } else {
            $result['callback'] = 'error';
